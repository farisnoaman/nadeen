    ...
    <UserCtx.Provider value={user}>
      <div className={`dashboard-shell ${collapsed ? 'sidebar-collapsed' : ''}`}>
        <aside id="dashboard-sidebar" className={mobile ? 'mobile-open' : ''} aria-label={t('workspace')}>
          ...
          <div className="workspace-switch" data-sidebar-label={user.companyName || t('personal')}>
            <span>{user.companyName?.split(' ').map(value => value[0]).slice(0, 2).join('') || 'FF'}</span>
            <div><small>{t('workspace')}</small><strong>{user.companyName || t('personal')}</strong></div>
            <ChevronDown />
          </div>
          <nav>
          ...
          <div className="side-bottom">
            <nav>
              <Link href="/dashboard/support" className={active('/dashboard/support') ? 'active' : ''} onClick={() => setMobile(false)} data-sidebar-label={t('support')} aria-label={t('support')}>
                <CircleHelp />
                <span>{t('support')}</span>
            ...
            <button type="button" className="side-user" data-sidebar-label={user.name} aria-label={user.name} onClick={() => { setProfile(!profile); setNotificationOpen(false); }}>
              ...
              <div><strong>{user.name}</strong><small>{user.role === 'company' ? t('administrator') : t('verifiedDriver')}</small></div>
              <ChevronDown />
            </button>
          </div>
        </aside>
        ...
        <main>
          <header className="dash-topbar">
            ...
            <div className="top-actions">
              ...
              {user.role === 'company' && <Link href="/dashboard/vehicles?new=1" className="btn primary top-add">+ {t('addVehicle')}</Link>}
