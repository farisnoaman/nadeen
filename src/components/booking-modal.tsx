'use client';
import { AlertTriangle, ArrowRight, CalendarDays, CheckCircle2, Clock3, Fuel, ShieldCheck, Tag } from 'lucide-react';
...
export function BookingModal({ vehicle, onClose }: { vehicle: any; onClose: () => void }) {
  ...
  useEffect(() => {
    if (vehicle) api(`/vehicles/${vehicle.id}`).then(setDetails);
  }, [vehicle]);
  ...
  const book = async () => {
    ...
    try {
      ...
      toast('Booking request sent with a protected 1-hour turnaround');
  ...
  return <Modal open={!!vehicle} onClose={onClose} title={t('bookTitle')} subtitle={`${vehicle.make} ${vehicle.model} · ${vehicle.companyName}`} wide>
    <div className="booking-modal enhanced-booking">
      ...
      <section className="booking-fields">
        ...
        <div className="price-breakdown"><div><span>{t('subtotal')}</span><strong>{money(subtotal)}</strong></div>{promo && <div className="discount"><span>{t('discount')}</span><strong>−{money(promo.discount)}</strong></div>}<div className="grand-total"><span>{t('total')}</span><strong>{money(promo?.total ?? subtotal)}</strong></div></div>
