'use client';
import { AlertTriangle, ArrowRight, Award, Baby, BriefcaseBusiness, CalendarDays, CheckCircle2, Clock3, MapPin, Minus, Plus, ShieldCheck, Tag, UserRound, Wifi } from 'lucide-react';
...
export function BookingModal({ vehicle, onClose }: { vehicle: any; onClose: () => void }) {
  ...
  const loyaltyDiscount = loyalty ? Math.min(
    Math.max(0, subtotal - Number(promo?.discount || 0)),
    subtotal * Number(loyalty.currentLevel?.discountPercentage || 0) / 100,
  ) : 0;
  const proposalTotal = subtotal + servicesTotal + protectionTotal - (promo?.discount || 0) - loyaltyDiscount;
  ...
  return <Modal open={!!vehicle} onClose={onClose} title={t('bookTitle')} subtitle={`${vehicle.make} ${vehicle.model} · ${vehicle.companyName}`} wide>
    <div className="booking-modal enhanced-booking">
      ...
      <section className="booking-fields">
        ...
        <div className="proposal-breakdown"><header><span>{t('rentalProposal')}</span><small>{t('pdfAfterConfirmation')}</small></header><div><span>{vehicle.make} {vehicle.model} · {quantity} {t(type)}{quantity > 1 ? 's' : ''}</span><strong>{money(subtotal)}</strong></div>{selectedProtection && <div><span>{selectedProtection.name || t(`protection_${selectedProtection.tier}`)}<small>{rentalDays} {t(rentalDays > 1 ? 'days' : 'day')} × {money(selectedProtection.dailyPrice)}</small></span><strong>{money(protectionTotal)}</strong></div>}{selectedServices.map(service => <div key={service.id}><span>{t(service.name)}<small>{service.days} {t(service.days > 1 ? 'days' : 'day')} × {money(service.dailyPrice)}</small></span><strong>{money(service.lineTotal)}</strong></div>)}{promo && <div className="discount"><span>{t('discount')} · {promo.promotion.code}</span><strong>−{money(promo.discount)}</strong></div>}{loyalty&&loyaltyDiscount>0&&<div className="discount loyalty-discount"><span>{t('loyaltyDiscount')} · {loyalty.currentLevel.name}<small>{loyalty.currentLevel.discountPercentage}% · {t('estimatedPointsEarned')}: +{estimatedLoyaltyPoints}</small></span><strong>−{money(loyaltyDiscount)}</strong></div>}<div className="grand-total"><span>{t('total')}</span><strong>{money(proposalTotal)}</strong></div></div>
