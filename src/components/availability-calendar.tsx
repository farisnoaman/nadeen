...
export function AvailabilityCalendar({ busyPeriods, startDate, endDate, onChange, onInvalid }: Props) {
  ...
  const select = (date: Date) => {
    ...
    while (walker <= date) {
      if (stateFor(walker).reserved) {
        onInvalid?.('That range crosses an existing reservation. Reserved dates cannot be selected.');
        return;
      }
  ...
  return <div className="availability-calendar">
    <header>
      <div><CalendarDays /><span><strong>Select rental dates</strong><small>Reserved dates are disabled automatically</small></span></div>
    ...
    <footer className="calendar-legend">
      ...
      <span><i className="buffer" /><Clock3 />1-hour turnaround</span>
    </footer>
  </div>;
}
