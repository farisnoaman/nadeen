...
const resetRequestState=()=>{ setCodeSent(false); setCode(''); setDemoCode(''); setCooldown(0); setRequestTimes([]); };
...
const requestCode=async()=>{
  ...
  try{const data:any=await api('/auth/phone/request',{method:'POST',body:JSON.stringify({phone:`+${dialCode}${national}`})});setPhone(data.phone.startsWith(`+${dialCode}`)?data.phone.slice(dialCode.length+1):national);setCodeSent(true);setDemoCode(data.demoCode||'');setCooldown(COOLDOWN_SECONDS);setRequestTimes([...recent,now])}
  catch(reason:any){setError(reason.message)}finally{setBusy(false)}
};
