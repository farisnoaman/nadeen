...
export async function GET(_: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const [selected] = await db.select({ vehicle: vehicles, companyName: companies.name, companyVerificationStatus:companies.verificationStatus, companySubscriptionStatus:companies.subscriptionStatus, companyOperationalStatus:companies.operationalStatus }).from(vehicles)
