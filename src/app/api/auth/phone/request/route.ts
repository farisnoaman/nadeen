...
export async function POST(request: Request) {
  try {
    ...
    if (!delivered) throw new Error('WhatsApp sign-in is not configured. Contact the platform administrator.');
    return ok({ sent: true, phone, expiresInSeconds: ttlSeconds });
  } catch (error) {
    return fail(error);
  }
}
