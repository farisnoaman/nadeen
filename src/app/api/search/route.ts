...
type SearchKind = 'vehicle' | 'rental' | 'maintenance' | 'maintenance_item' | 'promotion' | 'service' | 'support';
type AmountType = 'total' | 'cost' | 'perDay' | 'discount';
...
export async function GET(request: Request) {
  try {
    ...
    const [vehicleRows, rentalRows, promotionRows, serviceRows, supportRows] = await Promise.all([
    ...
    for (const row of vehicleRows as any[]) {
      ...
      add(candidates, {
        ...
        [vehicle.licensePlate, 1.5], [vehicle.make, 1.25], [vehicle.model, 1.25], [vehicle.trim, 1.1],
        ...
        [Array.isArray(vehicle.features) ? vehicle.features.join(' ') : '', 1], [vehicle.status, 1], [vehicle.odometer, 1], [row.companyName, 1],
    ...
    for (const row of rentalRows as any[]) {
      ...
      add(candidates, {
        ...
        [row.customerEmail, 1.15], [row.companyName, 1], [rental.status, 1], [rental.pickupLocation, 1],
